    ...
    int n, double* A, const double* C, int ldc, const double* S, int lds)
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        __m512d a_0_next = _mm512_loadu_pd(&A[5 * MR]);
        __m512d a_1_next = _mm512_loadu_pd(&A[5 * MR + 8]);
        __m512d a_2_next = _mm512_loadu_pd(&A[5 * MR + 16]);
        __m512d a_3_next = _mm512_loadu_pd(&A[5 * MR + 24]);

        // Apply rotation 0 to values 4 and 5 (next)
        ...
        // Apply rotation 1 to values 3 and 4 (next)
        ...
        // Apply rotation 2 to values 2 and 3 (next)
        ...
        // Apply rotation 3 to values 1 and 2 (next)
        ...
        // Apply rotation 4 to values 0 and 1 (next)
